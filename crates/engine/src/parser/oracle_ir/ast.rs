...
/// `AddRestriction`, `ForceAttack`, `ForceBlock`, `PreventDamage`. Read that
...
pub(crate) fn with_clause_chain_duration(
    ...
    while let Some(def) = link {
        if duration_governs(&def.effect)
            && (def.duration.is_none() || matches!(def.duration, Some(Duration::Permanent)))
        {
...
mod duration_distribution_tests_7923 {
    ...
    fn stamped(effect: &Effect) -> Effect {
        ...
        apply_duration_to_effect(&mut e, &outer());
        e
    }
    ...
    fn chain_duration_walks_governed_links_and_yields_to_explicit() {
        ...
        explicit.sub_ability = Some(Box::new(leaf));
        // link 2: carries the `Permanent` sub-parser sentinel -> overwritten.
        let mut sentinel = AbilityDefinition::new(AbilityKind::Spell, generic());
        ...
        let l4 = l3.sub_ability.as_deref().expect("leaf");
        assert_eq!(
            l4.duration, None,
